 ...
 * @author Lecrubier
 */
public class ArbreXMLCellEditor implements ActionListener, TreeCellEditor,
	TreeSelectionListener {

	/** Editor handling the editing. */
	protected TreeCellEditor realEditor;
	/** Renderer, used to get border and offsets from. */
	protected ArbreXMLCellRenderer renderer;
	...
	protected transient JTreeXML tree;
	 ...
	 */
	public ArbreXMLCellEditor(JTree tree,
		TreeCellRenderer renderer) {
		this((JTreeXML) tree, (ArbreXMLCellRenderer) renderer, null);
	}
	 ...
	 */
	public ArbreXMLCellEditor(JTreeXML tree, ArbreXMLCellRenderer renderer,	ArbreXMLCellEditorFenetre editor) {
...
//
	...
	public Component getTreeCellEditorComponent(JTree tree, Object value,
		...
		editingComponent = realEditor.getTreeCellEditorComponent(tree, ((ArbreXMLNode)tree.getSelectionPath().getLastPathComponent()).getElement().getText(),
...
//		tree.getModel().valueForPathChanged(lastPath, realEditor.getCellEditorValue());
			...
			if (event.getSource() instanceof JTree) {
				...
				if (event instanceof MouseEvent) {
					...
					if (path != null) {
						...
						ArbreXMLModel treeModel = tree.getModel();
...
//
	 ...
	 */
	protected void setTree(JTreeXML newTree) {
		if (tree != newTree) {
			if (tree != null) {
	...
	protected void determineOffset(JTree tree, Object value,
		...
		boolean leaf, int row) {
		if (renderer != null) {
			editingIcon = LanceurRayonApp.getInstance(LanceurRayon.LanceurRayonApp.class).getContext().getResourceMap(LanceurRayonApp.class).getImageIcon("EditionArbre.icon");
//			if (leaf) {
...
//			}
//			System.out.println(((ArbreXMLNode)value).isLeaf());
			if (editingIcon != null) {
...
//			editingContainer.add(editingComponent);
	...
	protected TreeCellEditor createTreeCellEditor() {
		...
		DefaultCellEditor editor = new DefaultCellEditor(new DefaultTextField(aBorder)) {
			...
			@Override
			public Object getCellEditorValue() {
				Element ancien = ((ArbreXMLNode)lastPath.getLastPathComponent()).getElement();
				
				String nom = ancien.getAttributeValue("Nom","Inconnu");
				String role = ancien.getAttributeValue("Role","Inconnu");
				
				if(ancien.getName().toLowerCase(Locale.ENGLISH).compareTo("double")==0) {
	
					try {
						Double a = Double.parseDouble(super.getCellEditorValue().toString());
						return new ArbreXMLNode(new Nombre(a).toJdom(nom,role));
					} catch (NumberFormatException ex) {
						Logger.getLogger("Journal").log(Level.WARNING, "Nombre Incorrect");
						return lastPath.getLastPathComponent();
					}
					
				} else if(ancien.getName().toLowerCase(Locale.ENGLISH).compareTo("integer")==0) {
					try {
						Integer a = Integer.parseInt(super.getCellEditorValue().toString());
						return new ArbreXMLNode(new Nombre(a).toJdom(nom,role));
					} catch (NumberFormatException ex) {
						Logger.getLogger("Journal").log(Level.WARNING, "Nombre Incorrect");
						return lastPath.getLastPathComponent();
...
// If not font has been set, prefer the renderers height.
			if (renderer != null &&
				ArbreXMLCellEditor.this.getFont() == null) {
