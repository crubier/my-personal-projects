 ...
 * @author Lecrubier
 */
public class ArbreXMLCellRenderer extends JLabel implements TreeCellRenderer{
...
//			expanded, leaf, row, hasFocus);
		...
		Element value = ((ArbreXMLNode) val).getElement();
		String stringValue = null;
		if(value.getAttributeValue("Reference","Inconnu").compareTo("Inconnu")!=0) {
			stringValue = value.getName()+ " " + value.getAttributeValue("Role","Inconnu") + " " + value.getAttributeValue("Nom", "") + " (" + value.getAttributeValue("Reference","Inconnue")+ ") ";
		}
		else {
			stringValue = value.getName()+ " " + value.getAttributeValue("Role","Inconnu") + " " + value.getAttributeValue("Nom", "") + " ";
		}
		
		
		java.util.List<Attribute> atts = (java.util.List<Attribute>)value.getAttributes();
		for(Attribute att : atts) {
			if(att.getName().toLowerCase(Locale.ENGLISH).compareTo("role") != 0 && att.getName().toLowerCase(Locale.ENGLISH).compareTo("nom") != 0 && att.getName().toLowerCase(Locale.ENGLISH).compareTo("reference") != 0) {
			stringValue = stringValue.concat(att.getName()).concat("='").concat(att.getValue()).concat("' ");
			}
		}
		if(value.getText()!= null)
		if(value.getTextNormalize().length()>0) {
			stringValue = stringValue.concat(" : ").concat(value.getText()).concat("");
		}
...
//		}
		...
		String nomImage = value.getName();
		if(nomImage.compareTo("Liste")== 0)  {
			if(value.getAttributeValue("Type","Inconnu").compareTo("Double")==0) {
				nomImage = "Liste_Double";
			} else if(value.getAttributeValue("Type","Inconnu").compareTo("Integer")==0) {
				nomImage = "Liste_Integer";
			} else if(value.getAttributeValue("Type","Inconnu").compareTo("Nombre")==0) {
				nomImage = "Liste_Nombre";
			} else {
				nomImage = "Liste";
			}
		}

		Icon icone = new ImageIcon("build/classes/LanceurRayon/IHM/Arbre/resources/icones/"+nomImage+".png");
		setIcon(icone);
