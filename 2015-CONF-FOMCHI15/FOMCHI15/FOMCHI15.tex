...
\author{%
  \alignauthor{Vincent Lecrubier\\
    \affaddr{ONERA DTIM/LAPS}\\
    \affaddr{Toulouse, France}\\
    \email{lecrubier@onera.fr}}\\
...
Flow-based representations maps well to systems whose data is defined on \emph{continuous} time intervals, such as the pressure inside a reactor. Examples of flow-based representations include Lustre \cite{halbwachs91}, Scade... 
...
In this section, we will use LIDL to describe the Boiling Water Reactor (BWR) use case. For the sake of simplicity, we will limit ourselves to an abstract interface as described in \cite{Paterno:2009:MUD:1614390.1614394}. However, LIDL is not restricted to the specification of abstract user interfaces.
