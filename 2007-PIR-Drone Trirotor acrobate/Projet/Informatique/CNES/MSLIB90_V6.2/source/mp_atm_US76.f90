...
real(pm_reel), dimension (nmax, 7), parameter :: rntab = reshape(rntab0, (/nmax,7/), order= (/2,1/))


intrinsic abs, exp, sqrt, dot_product, sum, real

character(len=pm_longueur_info_utilisateur) :: info_utilisateur = &
     '@(#) Fichier MSLIB mp_atm_US76.f90: derniere modification V6.0 >'

! Ne pas toucher a la ligne suivante
character(len=pm_longueur_rcs_id) :: rcs_id =' $Id: mp_atm_US76.f90,v 1.14 2004/06/14 15:13:00 mslibdev Exp $ '
...
if (altitude <= 86.e3_pm_reel) then
   ...
   xmol=dot_product(rnn(2:7), rmoltb(2:7))                  !          contribution a la somme totale de chaque "ni"
   rn = sum(rnn(2:7))
