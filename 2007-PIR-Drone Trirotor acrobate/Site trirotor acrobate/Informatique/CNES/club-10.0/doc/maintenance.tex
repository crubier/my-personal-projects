...
La maintenance de la bibliothèque \bibliotheque{club} s'effectue selon
quelques principes qui concernent la portabilité, les outils utilisés,
les procédures de maintenance, les fichiers descriptifs et l'archivage
sous \outil{cvs}.
...
produits \textsc{gnu}. Le document "GNU coding standards" est très
utile pour comprendre cette organisation
(cf~\ref{ref:gnu-coding-standards}).  Dans cet esprit, l'environnement
de maintenance nécessite beaucoup d'outils mais les utilisateurs
finaux n'ont guère besoin que de \outil{gunzip} pour décompresser la
...
Le développeur récupère tout d'abord le module club par une commande
\texttt{cvs checkout club}, il lui faut ensuite générer certains
...
\item[autoheader] génère le fichier \texttt{src/ClubConfig.h.in}
...
\begin{changebar}
Par défaut, la bibliothèque \bibliotheque{club} est générée sous forme
statique. Pour générer \bibliotheque{club} en version dynamique, il
faut utiliser l'option \texttt{-{}-enable-shared} du script
\texttt{configure}. Il faut cependant prendre garde que pour pouvoir
générer une bibliothèque sous forme dynamique, toutes les
bibliothèques dont elle dépend doivent être disponible sous cette
forme. Si on active les supports \bibliotheque{madona} et
\bibliotheque{xerces}, il faut vérifier qu'elles soient
disponibles en dynamique (c'est généralement le cas pour
\bibliotheque{xerces}, mais c'est généralement faux pour
\bibliotheque{madona}) avant d'activer l'option
\texttt{-{}-enable-shared}. On peut également désactiver la génération
de la bibliothèque statique à l'aide de l'option
...
gérés sous \outil{cvs} (cf.~\ref{ref:cvs}). Les fichiers pouvant être
générés automatiquement ne \emph{sont pas gérés sous \outil{cvs}}.
...
sont générés, leur suffixe .in signifie simplement qu'une fois générés
(par \texttt{autoheader} et par \outil{automake} respectivement) ils
servent de sources à \outil{autoconf} (qui génère alors
\texttt{src/ClubConfig.h} et \texttt{Makefile}). Les fichiers
éditables sont donc : \texttt{configure.ac}, et tous les
\texttt{Makefile.am}.
...
\texttt{src/ClubConfig.h} généré par configure, et c'est ce
\texttt{\#define} qui est utilisé par la fonction clubVersion.
