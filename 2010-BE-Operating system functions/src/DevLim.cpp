...
double devlim(int n , double x, int mode, int nbProcessus, int nbTests) {
	// n = ordre de la fonction de bessel
	// x = point d'evaluation

	filebuf fb;
	fb.open ("devlim.csv",ios::out);
	...
	for(int ff=0; ff< nbTests;ff++) {
		...
		if (numero == 0) {
			...
			while(recus < nbProcessus) {
				...
				for (int i = 1; i <nbproc; i++) {
					...
					if(read(pfd[0][i], &buffer, sizeof(buffer))==sizeof(buffer)){
						...
						// On modifie le resultat global grace au resultat du processus i
						signe = -1.* signe;

						fluxsortie << "Add	"<<i<<"	"<< signe <<"	"<<buffer<<"	"<<coef<<endl;

						// On ne prend pas en compte les resultats NaN ou Inf
						if(buffer!=0.0/0.0 && buffer !=-0.0/0.0 && buffer !=1.0/0.0 && buffer !=-1.0/0.0) {
							resultat = resultat + ((signe * buffer) / (coef * coef));
						}
			...
			double res =  (double) numero;	// Le resultat retourne par le processus (initialise a son numero)
			...
			// Boucle de calcul du resultat (facteur d'ordre <numero> du developpement limite de la fonction de bessel Jn , en x) ;
